...
| **Werkers per lab** | Een lab kan meerdere containers hebben die /workspace delen — één werkplaats met meer handen. Elke lopende planning bezet er één, dus met drie werkers lopen er drie planningen tegelijk, ook binnen hetzelfde bord. Op- en afschalen kan live; afschalen raakt alleen werkers die niets doen |
| **Wachten op lang werk** | Moet de agent wachten op een pipeline of deploy? Dan pauzeert de planning met een hervattijd (`board__wait_until`), komt de werker vrij voor ander werk, en wordt het ticket op tijd opnieuw opgepakt — met zijn eigen opmerkingen als context. De CLI-tools die dit zelf leken te kunnen (wekkers, cron) staan dicht, en een run die er tóch een gebruikt meldt dat op het ticket |
